...
`RIOT_API_KEY`와 `SUPABASE_SERVICE_ROLE_KEY`는 서버 환경변수에서만 읽습니다. 브라우저 코드나
...
6. `/api/health`에서 `hasRiotApiKey`, `hasSupabaseConfig`가 `true`인지 확인합니다.
...
2. `RIOT_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`를 Production 환경변수로 등록합니다.
